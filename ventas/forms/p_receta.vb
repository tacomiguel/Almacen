...
Public Class p_receta
    ...
    'codigo de grupo de articulos
    Private lcod_grupo As String = ""
    ...
    Private Sub txtCodigo_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtCodigo.LostFocus
        ...
        '    txtCodigo.Text = Microsoft.VisualBasic.Right("00000000" + Trim(txtCodigo.Text), 8)
        'End If
    End Sub
        ...
        Try
            If validaIngreso() Then
                ...
                If lcBoton = "Añadir" Then
                    cad = "Insert Into articulo(cod_art,cod_artext,cod_artext1,cod_artext2,cod_artext3,nom_art,nom_art2,cod_uni,cod_tart,cod_sgrupo,cod_grupov," &
                        "pre_costo,pre_venta,pre_inc_imp,afecto_igv,factor_prod,cod_alma,cod_area,es_pcosto,activo,b_color,foto)" &
                        "values('" &
                    txtCodigo.Text & "','" & TxtCodigoVta.Text & "','" & TxtCodigoVta1.Text & "','" & txtCodigoVta2.Text & "','" & txtCodigoVta3.Text & "','" & txtProduccion.Text & "','" & txtdsc_corta.Text & "','" & cboUnidad.SelectedValue & "','" &
                    cboTipo.SelectedValue & "','" & cboGrupo.SelectedValue & "','" & cboGventa.SelectedValue & "'," & txtPreCosto.Text & "," &
                    ...
                    ' cboTipo.SelectedValue & "','" & cboGrupo.SelectedValue & "','" & cboGventa.SelectedValue & "'," & txtPreCosto.Text & "," &
                Else
                    cad = "update articulo set cod_art='" & txtCodigo.Text & "'," & "nom_art='" & txtProduccion.Text & "'," &
                    "nom_art2='" & txtdsc_corta.Text & "',cod_uni='" & cboUnidad.SelectedValue & "'," &
                    "pre_costo=" & txtPreCosto.Text & ",pre_venta=" & txtPreVenta.Text & "," &
                    "cod_artext='" & TxtCodigoVta.Text & "',cod_artext1='" & TxtCodigoVta1.Text & "',cod_artext2='" & txtCodigoVta2.Text & "',cod_artext3='" & txtCodigoVta3.Text & "'," &
                    "pre_inc_imp=" & chkPreIncImp.CheckState & ",afecto_igv=" & chkPreAfecImp.CheckState & ",cod_area='" & cboArea.SelectedValue & "'," &
                    "cod_tart='" & cboTipo.SelectedValue & "',cod_sgrupo='" & cboGrupo.SelectedValue & "',cod_grupov='" & cboGventa.SelectedValue & "'," &
                    "factor_prod=" & TxtFactorProd.Text & ",es_pcosto=" & chkpcosto.CheckState & ",activo=" & chkActivo.CheckState & ",foto = ?imagen" & ",b_color='" & color & "'," &
                    "b_ancho=" & Val(txtancho.Text) & ",b_alto=" & Val(txtalto.Text) &
    ...
    Private Sub txtBuscarInsumo_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtBuscarInsumo.GotFocus
        ...
        '            Dim mCatalogo As New Catalogo
        '            dsInsumo = mCatalogo.recuperaProducto(pCatalogoXalmacen, cCatalogoDestino, txtBuscarInsumo.Text)
        '            dataInsumo.DataSource = dsInsumo.Tables("articulo").DefaultView
        '            cargaEstructuraInsumos()
        '            coloreaProducciones()
        '        End If
    ...
    Private Sub txtBuscarInsumo_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtBuscarInsumo.TextChanged
        If Not estaCargando Then
            ...
            If Len(txtBuscarInsumo.Text) > 3 Then
                dsInsumo = mCatalogo.recuperaProducto(pCatalogoXalmacen, cCatalogoDestino, txtBuscarInsumo.Text)
                'dsInsumo = mCatalogo.recuperaProducto(pCatalogoXalmacen, cCatalogoDestino, txtBuscarInsumo.Text)
                dataInsumo.DataSource = dsInsumo.Tables("articulo").DefaultView
                cargaEstructuraInsumos()
        ...
        Else
            If dataInsumo.RowCount > 0 Then
                If Len(txtCantidad.Text) > 0 Then
                    ...
                    txtCosto.Text = Round(lcosto * txtCantidad.Text, 5)
                Else
                    txtCantidad.Text = 0
                End If
            End If
        ...
        If dataProduccion.RowCount > 0 Then
            ...
            If tabreceta.IsSelected Then
                If mReceta.existeInsumoEnReceta(cod_rec, cod_art) Then
                    MessageBox.Show("El Insumo ya Esta Registrado en la Receta...", "SGA", MessageBoxButtons.OK, MessageBoxIcon.Information)
                Else
                    If cod_rec <> cod_art Then
                        If Val(txtCantidad.Text) > 0 Then
                            ...
                            txtBuscarInsumo.Focus()
                        Else
                            MessageBox.Show("Ingrese la Cantidad a Registrar...", "SGA", MessageBoxButtons.OK, MessageBoxIcon.Information)
                            txtCantidad.Focus()
                        End If
                    Else
                        MessageBox.Show("Una Receta NO puede ser Insumo de su Propia Receta...", "SGA", MessageBoxButtons.OK, MessageBoxIcon.Information)
                    End If
                End If
            Else
                If DataCombo.RowCount > 0 Then
                    Try
                        ...
                        If cod_combo <> cod_art Then
                            ...
                            End If
                        Else
                            MessageBox.Show("Un Combo NO puede ser Insumo del mismo Combo...", "SGA", MessageBoxButtons.OK, MessageBoxIcon.Information)
                        End If
    ...
    Sub calcularcostoreceta(ByVal cod_art As String, ByVal factor As Decimal)
    ...
    Sub cargaEstructuraReceta()
        With dataReceta
            ...
            .Columns("cant").DefaultCellStyle.Format = "N" & 6
            .Columns("pre_costo").HeaderText = "Costo"
            .Columns("pre_costo").Width = 60
            .Columns("pre_costo").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
            .Columns("pre_costo").DefaultCellStyle.Format = "N" & 6
            .Columns("total").HeaderText = "Monto"
            .Columns("total").Width = 60
            .Columns("total").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
            .Columns("total").DefaultCellStyle.Format = "N" & 6
    ...
    Private Sub tvCombo_DoubleClick(sender As Object, e As System.EventArgs) Handles tvCombo.DoubleClick
        ...
        If DataCombo.RowCount > 0 Then
            Try
                Dim mReceta As New Receta
                Dim rpta As Integer
                rpta = MessageBox.Show("Esta Seguro de Quitar el Item del Combo...", "SGA", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
                If rpta = 6 Then
    ...
    Private Sub cmdEliminaCombo_Click(sender As System.Object, e As System.EventArgs) Handles cmdEliminaCombo.Click
        If DataCombo.RowCount > 0 Then
            Try
                ...
                Dim mReceta As New Receta
                Dim rpta As Integer
                rpta = MessageBox.Show("Esta Seguro de Quitar el Combo...", "SGA", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
                If rpta = 6 Then
                    DataCombo.Rows(0).Selected = True
                    Dim cod_combo As String = DataCombo("cod_combo", DataCombo.CurrentRow.Index).Value
    ...
    Private Sub cboGrupo_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cboGrupo.SelectedIndexChanged
        If Not estaCargando Then
            If cboGrupo.SelectedValue <> "" Then
                Dim fcod_grupo As String = cboGrupo.SelectedValue.ToString
                Dim msubgrupo As New subgrupo
                lcod_grupo = msubgrupo.devuelveGrupo(fcod_grupo)
            End If
        End If
    End Sub

    Private Sub cmdPromedio_Click(sender As Object, e As EventArgs)

    End Sub

    Private Sub SimpleButton2_Click(sender As Object, e As EventArgs) Handles cmdCosto.Click
        ActualizarCostos()

    End Sub
    Sub ActualizarCostos()
        Dim I, nroReg As Integer
        Dim factor As Decimal
        Dim cod_art As String
        Dim cAlmacen As String = cboAlmacen.SelectedValue
        If dataProduccion.RowCount > 0 Then
            nroReg = dataProduccion.RowCount

            For I = 0 To nroReg - 1

                cod_art = dataProduccion.Item("cod_art", I).Value
                factor = dataProduccion.Item("factor_prod", I).Value
                muestraReceta(cod_art)
                calcularcostoreceta(cod_art, factor)

                cmdCosto.Text = I + 1 & " de " & nroReg

                cmdCosto.Refresh()

            Next
            cmdCosto.Text = "Actualiza Costos"
    ...
    Private Sub dataProduccion_CellParsing(sender As Object, e As DataGridViewCellParsingEventArgs) Handles dataProduccion.CellParsing
